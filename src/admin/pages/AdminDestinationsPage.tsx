...
export default function AdminDestinationsPage() {
  ...
  async function uploadDestinationImage(file: File | undefined) {
    if (!file) return;
    setUploading(true);
    try {
        ...
        >
          ...
          <label className="flex flex-col gap-1 text-sm font-semibold">
            ...
            <input
              type="file"
              accept="image/*"
              onChange={(e) => uploadDestinationImage(e.target.files?.[0])}
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-t border-gray-100">
                ...
                <td className="px-4 py-3">{item.properties ?? 0}</td>
